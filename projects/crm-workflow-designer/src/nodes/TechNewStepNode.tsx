...
export function TechNewStepNode({ data, selected }: NodeProps) {
  ...
  return (
    <div style={containerStyle(selected ?? false)}>
      <Handle type="target" position={inPos} id="in" style={handleStyle('#94a3b8')} />
      ...
      <Handle type="source" position={outPos} id="out" style={handleStyle('#64748b')} />
    </div>
  );
}
...
const techBadge: React.CSSProperties = {
  fontSize: 8, fontWeight: 800, color: '#94a3b8', letterSpacing: '0.1em', flexShrink: 0,
};
...
const divider: React.CSSProperties = { borderTop: '1px solid #f1f5f9', margin: '4px 12px 4px' };
