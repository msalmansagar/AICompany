...
const PALETTE: Record<'dark' | 'light', SectionPalette> = {
  dark: {
    header: { ...baseHeader, color: 'var(--text-disabled)' },
    caret: { fontSize: 10, color: 'var(--text-secondary)' },
    summaryBadge: { ...baseBadge, color: 'var(--accent-route)', background: 'var(--primary-tint)', border: '1px solid var(--accent-route)' },
    body: baseBody,
    notice: {
      fontSize: 10, color: 'var(--accent-route)', background: 'var(--primary-tint)', border: '1px solid var(--accent-route)',
      borderRadius: 4, padding: '6px 8px', lineHeight: 1.4,
    },
    field: baseField,
    label: { ...baseLabel, color: 'var(--text-secondary)' },
    select: { ...baseSelect, background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)' },
    hint: { fontSize: 10, color: 'var(--text-secondary)', lineHeight: 1.4 },
  },
  light: {
    header: { ...baseHeader, color: 'var(--text-secondary)' },
    caret: { fontSize: 10, color: 'var(--text-disabled)' },
    summaryBadge: { ...baseBadge, color: 'var(--accent-route)', background: 'var(--accent-route-bg)', border: '1px solid var(--accent-route)' },
    body: baseBody,
    notice: {
      fontSize: 10, color: 'var(--accent-route)', background: 'var(--accent-route-bg)', border: '1px solid var(--accent-route)',
      borderRadius: 4, padding: '6px 8px', lineHeight: 1.4,
    },
    field: baseField,
    label: { ...baseLabel, color: 'var(--text-secondary)' },
    select: { ...baseSelect, background: 'var(--surface)', border: '1px solid var(--border-strong)', color: 'var(--text)' },
    hint: { fontSize: 10, color: 'var(--text-secondary)', lineHeight: 1.4 },
  },
};
