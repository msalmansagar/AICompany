...
export const MEASURES: readonly MeasureEntry[] = [
  ...
  { code: 'M-ACTIVITIES-BY-AGE', title: 'Open activities by age', classification: 'factual', grain: 'ACTIVITY', source: 'qdb_collectionactivity · createdon', semantics: 'Open activities in bands by days since they were created. Age since creation is a fact; "overdue" is not, because no TAT start policy exists (KI-101).' },
...
export const REPORT_DEFINITIONS: readonly ReportDefinitionEntry[] = [
  ...
  { code: 'DCP-RPT-007', title: 'Open activities by owner', purpose: 'Work held per owner, and how much is unassigned', audience: ['supervisor'], kind: 'report', grain: 'ACTIVITY', source: 'qdb_collectionactivity · ownerid, statecode', dimensions: ['sourceSystem', 'activityType'], measures: ['M-ACTIVITIES-BY-OWNER', 'M-ASSIGNMENT-STATE'], drillDown: 'queues', security: 'canexecute for DCP supervisor role', freshness: 'CRM query time', status: 'planned' },
  { code: 'DCP-RPT-008', title: 'Activities by type', purpose: 'What kind of work is recorded, in a window', audience: ['supervisor', 'management'], kind: 'report', grain: 'ACTIVITY', source: 'qdb_collectionactivity · qdb_activitytypeid, createdon', dimensions: ['sourceSystem', 'owner', 'activityState', 'dateFrom', 'dateTo'], measures: ['M-ACTIVITIES-BY-TYPE'], drillDown: 'queues', security: 'canexecute for DCP roles', freshness: 'CRM query time', status: 'planned' },
  { code: 'DCP-RPT-009', title: 'Open activities by age', purpose: 'How long open work has been open, in bands', audience: ['supervisor'], kind: 'report', grain: 'ACTIVITY', source: 'qdb_collectionactivity · createdon, statecode', dimensions: ['sourceSystem', 'owner', 'activityType'], measures: ['M-ACTIVITIES-BY-AGE', 'M-ESCALATED'], drillDown: 'queues', security: 'canexecute for DCP supervisor role', freshness: 'CRM query time', status: 'planned' },
  { code: 'DCP-RPT-010', title: 'Activities by month', purpose: 'Recorded activity volume over time', audience: ['management'], kind: 'report', grain: 'ACTIVITY', source: 'qdb_collectionactivity · createdon', dimensions: ['sourceSystem', 'activityType', 'dateFrom', 'dateTo'], measures: ['M-ACTIVITIES-BY-MONTH'], drillDown: 'none', security: 'canexecute for DCP roles', freshness: 'CRM query time', status: 'planned' },
  { code: 'DCP-RPT-011', title: 'Promises by status', purpose: 'The promise book as officers recorded it', audience: ['supervisor', 'management'], kind: 'report', grain: 'PTP', source: 'qdb_collectionactivity with a PTP date', dimensions: ['sourceSystem', 'owner', 'dateFrom', 'dateTo'], measures: ['M-PROMISES-BY-STATUS'], drillDown: 'promises', security: 'canexecute for DCP roles', freshness: 'CRM query time; statuses are officer-recorded, not verified payments', status: 'planned' },
  { code: 'DCP-RPT-012', title: 'Identity exceptions', purpose: 'Intake identity problems by status and reason', audience: ['operations'], kind: 'report', grain: 'EXCEPTION', source: 'qdb_identityexception', dimensions: ['dateFrom', 'dateTo'], measures: ['M-IDENTITY-EXCEPTIONS'], drillDown: 'intake', security: 'canexecute for DCP operations and manager roles', freshness: 'CRM query time', status: 'planned' },
  { code: 'DCP-RPT-013', title: 'Delinquency observations', purpose: 'Stored MIS observations over time, per source system', audience: ['management'], kind: 'report', grain: 'SNAPSHOT', source: 'qdb_delinquencysnapshot', dimensions: ['sourceSystem', 'dateFrom', 'dateTo'], measures: ['M-SNAPSHOT-OBSERVATIONS'], drillDown: 'intake', security: 'canexecute for DCP manager role', freshness: 'Each point is one stored observation (qdb_snapshotdate); the ARR book has one', status: 'planned' },
  { code: 'DCP-RPT-014', title: 'Configuration gaps', purpose: 'What the platform cannot do because configuration is absent', audience: ['operations'], kind: 'report', grain: 'CONFIGURATION', source: 'configuration entities', dimensions: [], measures: ['M-CONFIG-GAPS', 'M-CASES-NO-CUSTOMER', 'M-CASES-UNBUCKETED'], drillDown: 'none', security: 'canexecute for DCP operations and manager roles', freshness: 'CRM query time', status: 'planned' },
  { code: 'DCP-DB-001', title: 'Portfolio Overview', purpose: 'Management view of the open book', audience: ['management'], kind: 'dashboard', grain: 'CASE', source: 'DCP-RPT-001, 002, 003, 004, 006', dimensions: ['sourceSystem'], measures: ['M-OPEN-CASES', 'M-CURRENT-ARREARS', 'M-LOAN-BALANCE', 'M-DISTINCT-CUSTOMERS'], drillDown: 'cases', security: 'canexecute for DCP manager role', freshness: 'Stored MIS position', status: 'planned' },
