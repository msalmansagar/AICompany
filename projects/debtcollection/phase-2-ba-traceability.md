...
| FR-003 | 1 — Customer & Loan 360 | P1 | facts-and-analysis §5; non-negotiable | Query `qdb_delinquencysnapshot` for a facility; verify MIS batchReference and asOf on every row |
...
| FR-005 | 1 — Customer & Loan 360 | P1 | skeleton §A | Verify legal case status, insurance claim status, and deceased flag rendered on Customer 360 |
| FR-006 | 1 — Customer & Loan 360 | P1 | skeleton §A | Verify open and historical PTPs, restructure cases, and disputes linked to customer are visible |
...
| FR-008 | 1 — Customer & Loan 360 | P1 | non-negotiable R-06 | Open unresolved-identity queue; verify rows for: missing QID, name mismatch, QID in one org only |
| FR-009 | 1 — Customer & Loan 360 | P2 | non-negotiable R-01; facts §6.3 | With BFD flag ON, verify Customer 360 for C-1002 shows HL + BFD facilities merged |
| FR-010 | 1 — Customer & Loan 360 | P1 | facts-and-analysis §5 | Simulate MIS ingest failure; verify staleness alert displayed on Customer 360 |
| FR-011 | 1 — Customer & Loan 360 | P1 | skeleton §7.C | Verify payment history (date, amount, channel, rejection reason) rendered per facility |
...
| FR-013 | 1 — Customer & Loan 360 | P1 | skeleton §L | Verify dispute history rendered on Customer 360 |
...
| FR-015 | 1 — Customer & Loan 360 | P1 | facts-and-analysis §3 | Verify DPD bucket colour-coding uses 10-value taxonomy consistently |
...
| FR-021 | 2 — Delinquency & Case Creation | P2 | D-2; facts §2.2 | Set auto-creation threshold = 30 DPD in admin config; verify case auto-created on next ingest |
...
| FR-024 | 2 — Delinquency & Case Creation | P1 | non-negotiable; facts §5 | Open case screen; verify snapshot DPD and live DPD both displayed with "as of" label |
...
| FR-026 | 2 — Delinquency & Case Creation | P1 | facts §2.2 | Filter case list to >2000 DPD; verify distinct colour and separate queue segment |
...
| FR-030 | 2 — Delinquency & Case Creation | P2 | Q-05; ADR-DCP-02 | Enable BFD feature flag; create BFD case; verify router sends to BFD CRM not HL CRM |
...
| FR-049 | 5 — Collection Action Plan | P1 | skeleton §E | Escalate case to supervisor from action plan screen; verify Supervisor Review action created and supervisor notified |
| FR-050 | 5 — Collection Action Plan | P2 | skeleton §E | Apply action plan template to case; verify template-defined actions created |
| FR-051 | 5 — Collection Action Plan | P1 | skeleton §E; skeleton §6 | Return Supervisor Review action with reason; verify outcome captured in audit |
...
| FR-054 | 5 — Collection Action Plan | P1 | D-8; ADR-DCP-01 constraint 5 | Verify "Field Visit" appears as a selectable action type in the action creation form |
...
| FR-060 | 6 — PTP Management | P1 | skeleton §F controls | Reschedule PTP 3rd time (threshold=2); verify Senior Manager approval required |
...
| FR-063 | 6 — PTP Management | P1 | skeleton §10 KPIs | Log in as Senior Manager; verify PTP kept-rate per officer visible on performance dashboard |
...
| FR-071 | 7 — Communication Management | P1 | skeleton §G controls | Activate a template without Compliance approval; verify activation blocked |
| FR-072 | 7 — Communication Management | P1 | Q-11; skeleton §G | Send official letter; set status to Delivered-Signed; verify status persisted |
...
| FR-077 | 8 — Restructuring / Workout | P1 | skeleton §H; ADR-DCP-03 | Officer creates restructure proposal WO-3007; verify all mandatory fields enforced |
| FR-078 | 8 — Restructuring / Workout | P1 | skeleton §H approval workflow | Approve restructure at Senior Manager stage; verify status advances to Head of Collections stage |
| FR-079 | 8 — Restructuring / Workout | P1 | skeleton §12 | Change approval threshold in admin config; verify new threshold active without deployment |
| FR-080 | 8 — Restructuring / Workout | P1 | skeleton §6 | Submit restructure with waiver; verify separate waiver approval request created |
| FR-081 | 8 — Restructuring / Workout | P1 | skeleton §H | Submit restructure without uploading mandatory document; verify submission blocked |
| FR-082 | 8 — Restructuring / Workout | P2 | skeleton §H | Breach restructured instalment; verify case flagged as Re-Defaulted |
| FR-083 | 8 — Restructuring / Workout | P1 | skeleton §H | Submit restructure without financial assessment; verify mandatory field validation error |
| FR-084 | 8 — Restructuring / Workout | P1 | skeleton §H | Reject restructure without reason; verify rejection blocked |
| FR-085 | 8 — Restructuring / Workout | P1 | non-negotiable R-03 | Approve a restructure; verify audit log entry with stage, approver, timestamp |
| FR-086 | 8 — Restructuring / Workout | P1 | ADR-DCP-03 decision 3 | Log in as Collection Officer; verify restructure case status visible read-only on portal |
| FR-087 | 9 — Legal Case Management | P1 | skeleton §I; Final Requirements | Officer submits legal referral with all checklist items; verify `qdb_legalcase` created |
| FR-088 | 9 — Legal Case Management | P1 | skeleton §I controls | Submit legal referral without Senior Manager approval; verify submission blocked |
| FR-089 | 9 — Legal Case Management | P1 | skeleton §I controls | Submit legal referral for case below DPD threshold; verify router plugin blocks |
| FR-090 | 9 — Legal Case Management | P1 | skeleton §I; ADR-DCP-03 | Officer views legal case status (Case Filed); verify status displayed read-only; no edit controls present |
...
| FR-092 | 9 — Legal Case Management | P1 | skeleton §I controls | Legal User returns case from native CRM; verify `qdb_collectionaction` created and case status updated |
| FR-093 | 9 — Legal Case Management | P1 | skeleton §I | Update legal recovery amount in native CRM; verify value reflected in portal read-only view |
| FR-094 | 9 — Legal Case Management | P1 | skeleton §I | Upload court document; verify audit log records document upload with actor and timestamp |
...
| FR-098 | 10 — Deceased & Insurance | P1 | skeleton §J | Create insurance claim CLM-401; verify mandatory fields (kind, insurer, date, documents) enforced |
| FR-099 | 10 — Deceased & Insurance | P1 | skeleton §J; Final Requirements | Advance claim status from Submitted to Under Review; verify audit trail updated |
| FR-100 | 10 — Deceased & Insurance | P1 | skeleton §J controls; Q-13 | Attempt to send communication to heir without Senior Manager approval; verify blocked |
| FR-101 | 10 — Deceased & Insurance | P1 | skeleton §J | Mark claim as Paid; verify outstanding balance updated on facility record and Customer 360 |
...
| FR-103 | 12 — Disputes | P1 | skeleton §L; Final Requirements | Register dispute DSP-301; verify mandatory fields: type, root cause, owner, SLA date |
| FR-104 | 12 — Disputes | P1 | skeleton §L; prototype DSP | Register dispute for HL-CC-20399; verify `collectionPaused = true` on the case |
| FR-105 | 12 — Disputes | P1 | skeleton §L | Add investigation note; verify captured in audit trail |
| FR-106 | 12 — Disputes | P1 | skeleton §L; skeleton §12 | Let SLA due date pass; verify dispute appears in supervisor queue as overdue |
| FR-107 | 12 — Disputes | P1 | skeleton §L; Final Requirements | Verify all 7 standard complaint types available as configurable options |
...
| FR-115 | 13 — Audit Trail & RBAC | P1 | skeleton §11 | Log in and log out; verify login/logout events in audit trail |
| FR-116 | 14 — Admin Configuration | P1 | skeleton §12 | Admin changes SLA threshold for High Risk queue; verify new threshold active immediately, no deployment |
| FR-117 | 14 — Admin Configuration | P1 | skeleton §G controls | Create template without Compliance approval; verify template not available to officers |
...
| FR-124 | 15 — Integration Layer | P2 | skeleton §8.E | Run QCB monthly file generation; verify file produced in correct format |
| FR-125 | 15 — Integration Layer | P1 | skeleton §8.F; facts §2 | Run DWH nightly extract; verify all collection entity types included in output |
...
| FR-128 | 12 — Dashboards & MIS | P1 | skeleton §9.B; facts §2 | Open Portfolio Dashboard; verify data sourced from DWH extract, not live CRM query |
...
| FR-130 | 12 — Dashboards & MIS | P1 | skeleton §9.D | Open Legal Dashboard; verify cases displayed by legal stage with aging |
| FR-131 | 12 — Dashboards & MIS | P1 | skeleton §9.G; SO-02 | Open Management Dashboard; verify NPL trend, monthly recoveries, officer productivity |
| FR-132 | 12 — Dashboards & MIS | P1 | facts §2.2 | Verify >2000 DPD population shown in a segregated section on Operational Dashboard |
...
|---|---|---|---|---|
| SC-01 | A Collection Officer can open a customer's full HL facility view, log an action, and capture a PTP from one screen without leaving the portal | FR-001, FR-002, FR-003, FR-045, FR-046, FR-055 | Yes | FR-001/002/003 deliver the facility view; FR-045/046 deliver action logging; FR-055 delivers PTP capture |
