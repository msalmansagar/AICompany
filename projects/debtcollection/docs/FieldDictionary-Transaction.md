...
| Facility Source System | qdb_facilitysourcesystem | string | 50 | O | — | M | N | RO | **Added 2026-09-18 (KI-47, approved at the Phase 2 gate).** The other half of the canonical facility identity: a facility number means nothing without the system that issued it. Deliberately **not** ApplicationRequired — historical rows predate the column — and deliberately **not** the sole source of the source system, which the snapshot key composition may also carry independently. | Y | Y | CbD | CbD | sourceSystem | new | Mirrors qdb_collectioncase.qdb_facilitysourcesystem |
