...
| Q-05 | ✅ **ANSWERED 2026-09-12: TWO ORGS, HOUSING LOAN FIRST.** Housing Loan CRM and BFD CRM stay separate. Phase 1 delivers Housing Loan only with BFD behind a feature flag; the router and the QID identity map are built for both from day one. Tech doc v2.0's single-org model is the HL-side baseline, not the architecture | Tech doc v2.0 (single-org) vs the brief's dual-org router architecture |
| Q-06 | ✅ **ANSWERED 2026-09-14: MIS via the Middleware API.** QDB's data warehouse is called **MIS**; it is exposed through a Middleware API that returns delinquency data already classified into buckets (confirms D-1). The nightly CBS batch in tech doc v2.0 is superseded; `QDB.DCP.DelinquencyPlugin` is not built | Tech doc vs Final Requirements; user answer 2026-09-14 |
| Q-07 | ✅ **ANSWERED 2026-09-12: BUILD.** `Debt Collection Platform - SOW.docx` is an RFP to procure a vendor platform; the decision is to build in-house. ADR-DCP-01/02/03 stand as written; the RFP is treated as a superseded procurement draft, and its acceptance criteria are inputs to the BRD, not a contract | RFP vs tech doc vs this project |
...
| Q-14 | ✅ **ANSWERED 2026-09-14: BOTH ON-PREMISE NOW, BOTH MIGRATING TO DATAVERSE CLOUD IN ~18 MONTHS (≈ Q1 2028).** Consequences for the architect: (a) router authentication is a pluggable adapter — AD FS / OAuth against on-prem 9.x today, Azure AD MSAL service principal after migration — with no business logic aware of which; (b) only CRM features that exist on BOTH on-prem 9.x and Dataverse may be used (plugins, custom activities, classic workflows, Web API OData v4 — yes; cloud-only surfaces such as Custom APIs, Dataverse elastic tables, Power Automate as a hard dependency — no, or behind an abstraction); (c) the Node router is hosted on client infrastructure beside the CRMs first, containerised so the same image moves to Azure; (d) the schema and solution must import cleanly into Dataverse — no on-prem-only components; (e) the migration itself is OUT of DCP-001 scope but is a named non-functional constraint | Tech doc v2.0 hedges; user answer 2026-09-14 |
| COND-DCP-006 | ✅ **ANSWERED 2026-09-14: a DWH EXISTS — it is MIS.** Portfolio MIS dashboards (FR-125, FR-128) stay P1; the counterparty is the **Middleware API in front of MIS**, never a direct warehouse connection. The same API is the delinquency feed (Q-06), so one integration serves both the case pipeline and the portfolio MIS; the bucket taxonomy conflict (Q-03, §3) is therefore decided on the MIS side and the CRM option set must follow it | BRD CEO gate; user answer 2026-09-14 |

**Q-05 and Q-07 are the two that change everything downstream** (both now closed: build, two orgs with Housing Loan first; Q-14 also closed: on-prem now, cloud in ~18 months). A single-org build with UCI forms
