...
{
    /// <summary>
    /// Tests for the contact-hold guard added to
    ...
    public sealed class StatusTransitionValidatorStopContactTests
    {
        ...
        private const string EntityCustomer = "contact";
        private const string HoldAttribute = "qdb_contacthold";
        private const string StepConfiguration = "holdAttribute=" + HoldAttribute;

        // ── Guard blocks a customer on hold entering a contact-bearing state ──

        [Fact]
        public void ValidateTransition_CustomerOnHold_TransitionIntoInProgress_Rejected()
        {
            // Arrange
            ...
            SetupCustomerRetrieve(builder, customerId, isOnHold: true);
            ...
            Assert.Contains("contact hold", ex.Message);
        }

        // ── DeceasedInsuranceReview is not contact-bearing — always reachable ──────

        [Fact]
        public void ValidateTransition_CustomerOnHold_TransitionIntoDeceasedInsuranceReview_Allowed()
        {
        ...
        [Fact]
        public void ValidateTransition_CustomerOnHold_TransitionIntoReferredToLegal_Allowed()
        {
        ...
        [Fact]
        public void ValidateTransition_CustomerNotOnHold_ContactBearingTransition_Allowed()
        {
            // Arrange
            var customerId = Guid.NewGuid();
            var builder = BuildCaseWithCustomer(
                ...
                StatusTransitionMatrix.CaseStatus.InProgress,
                preImageCustomerId: customerId);
            SetupCustomerRetrieve(builder, customerId, isOnHold: false);
        ...
        [Fact]
        public void ValidateTransition_CaseWithNoCustomerId_ContactHoldGuard_Skipped()
        {
            // Arrange — pre-image and target both lack qdb_customerid
        ...
        public void ValidateTransition_CustomerIdInPreImage_RetrievesUsingPreImageId()
        {
            ...
            var preImage = new Entity(EntityCase);
            preImage["statuscode"] = new OptionSetValue(StatusTransitionMatrix.CaseStatus.Assigned);
            preImage["qdb_customerid"] = new EntityReference(EntityCustomer, preImageCustomerId);
            builder.WithPreImage(preImage);

            var target = new Entity(EntityCase);
            target["statuscode"] = new OptionSetValue(StatusTransitionMatrix.CaseStatus.InProgress);
            target["qdb_customerid"] = new EntityReference(EntityCustomer, targetCustomerId);
            builder.WithTarget(target);

            SetupCustomerRetrieve(builder, preImageCustomerId, isOnHold: false);
        ...
        private static void SetupCustomerRetrieve(
            PluginContextBuilder builder, Guid customerId, bool isOnHold)
        {
            var customer = new Entity(EntityCustomer, customerId);
            customer[HoldAttribute] = isOnHold;
            builder.MockService
        ...
        private static StatusTransitionValidator CreateValidator(PluginContextBuilder builder) =>
            new StatusTransitionValidator(builder.PluginContext, ContactHoldSettings.Parse(StepConfiguration));
    }
}