...
namespace Qdb.DebtCollection.Plugins.Plugins
{
    ...
    ///
    /// 🔴 LEGACY — NOT PART OF THE CANONICAL SCHEMA. The canonical schema records business
    /// history through the platform's native audit, so there is no <c>qdb_</c> audit table and
    /// this writer has no step in the QDB registration. It is retained, with its tests, because
    /// the earlier <c>msst_</c> deployment still runs it and that deployment has not been retired.
    /// Its removal is a decision for the Phase 1 gate, not something to do quietly.
    ///
