...
const SOLUTION_FRIENDLY_NAME = 'Maqsad DXP Platform';
...
export async function provisionSolution(
  ...
  if (existingSolutions.length > 0) {
    ...
    if (!solution) throw new Error('[PHASE-3] Solution query returned empty array element.');
    console.log(`[PHASE-3] [SKIP] Solution '${SOLUTION_UNIQUE_NAME}' already exists (v${solution.version}).`);
    return { name: SOLUTION_UNIQUE_NAME, status: 'skipped', id: solution.solutionid, solutionId: solution.solutionid };
  }

  if (env.DRY_RUN) {
    console.log(`[PHASE-3] [DRY-RUN SKIP] Would create solution '${SOLUTION_UNIQUE_NAME}' v${SOLUTION_VERSION}.`);
    return { name: SOLUTION_UNIQUE_NAME, status: 'dry-run', id: '', solutionId: '' };
  }
  ...
  return { name: SOLUTION_UNIQUE_NAME, status: 'created', id: solutionId, solutionId };
}
