...
// CHALLENGE 7 resolution:
// When publisher is absent and DRY_RUN=true, log a mock and return a fake publisherId
// so subsequent dry-run phases can proceed without real data.
...
export async function runPublisherCheck(
  ...
  if (publishers.length > 0) {
    ...
    if (!publisher) {
      throw new Error('[PHASE-2a] Publisher query returned empty array element.');
    }
    console.log(
      `[PHASE-2a] [PASS] Publisher found: ${publisher.friendlyname} (${publisher.publisherid})`,
    );
  ...
  if (!publisherId) {
    throw new Error(
      `[PHASE-2a] [ERROR] Publisher POST succeeded but OData-EntityId header was missing or unparseable. ` +
