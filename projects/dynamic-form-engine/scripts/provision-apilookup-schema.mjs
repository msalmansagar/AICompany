/**
 * DFE-APILOOKUP-001 — external-API lookup source columns on qdb_form_lookup_config (additive):
 *   qdb_lookup_source              String(10)   'entity' (default/null) | 'api'
 *   qdb_lookup_api_endpoint_key    String(100)  key resolving to a server-side registered endpoint
 *   qdb_lookup_api_value_path      String(200)  dot-path to the value in each API response item
 *   qdb_lookup_api_label_path      String(200)  dot-path to the label in each API response item
 *   qdb_lookup_api_search_param    String(100)  query param carrying the typed term (typeahead)
 *   qdb_lookup_api_search_mode     String(20)   'typeahead' | 'fetchAll'
 *
 * Run: node scripts/provision-apilookup-schema.mjs   (requires DV_CLIENT_SECRET)
...
async function run() {
  console.log(`DFE-APILOOKUP-001 schema provisioning\nOrg: ${DATAVERSE_URL}  Solution: ${SOLUTION_NAME}\n${'─'.repeat(56)}`);
  const token = await acquireToken();
  await addAttribute(token, 'qdb_lookup_source', stringAttr('qdb_lookup_source', 'Lookup Source', 10));
  await addAttribute(token, 'qdb_lookup_api_endpoint_key', stringAttr('qdb_lookup_api_endpoint_key', 'Lookup API Endpoint Key', 100));
  await addAttribute(token, 'qdb_lookup_api_value_path', stringAttr('qdb_lookup_api_value_path', 'Lookup API Value Path', 200));
  await addAttribute(token, 'qdb_lookup_api_label_path', stringAttr('qdb_lookup_api_label_path', 'Lookup API Label Path', 200));
  await addAttribute(token, 'qdb_lookup_api_search_param', stringAttr('qdb_lookup_api_search_param', 'Lookup API Search Param', 100));
  await addAttribute(token, 'qdb_lookup_api_search_mode', stringAttr('qdb_lookup_api_search_mode', 'Lookup API Search Mode', 20));
  console.log(`${'─'.repeat(56)}\nqdb_form_lookup_config API-source columns ensured.`);
}
