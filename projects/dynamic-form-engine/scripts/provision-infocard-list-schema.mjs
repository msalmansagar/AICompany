/**
 * DFE-INFOLIST-001 — configurable list styles on the inline info-card field (additive):
 *   qdb_info_card_list_type   String(20)   'bullet' | 'numbered-arabic' | 'numbered-roman'
 *   qdb_info_card_list_marker String(20)   'circle' | 'plain' | 'none'
 *
 * Run: node --env-file=scripts/.env scripts/provision-infocard-list-schema.mjs
...
async function run() {
  console.log(`DFE-INFOLIST-001 schema provisioning\nOrg: ${DATAVERSE_URL}  Solution: ${SOLUTION_NAME}\n${'─'.repeat(56)}`);
  const token = await acquireToken();
  await addAttribute(token, 'qdb_info_card_list_type', stringAttr('qdb_info_card_list_type', 'Info Card List Type', 20));
  await addAttribute(token, 'qdb_info_card_list_marker', stringAttr('qdb_info_card_list_marker', 'Info Card List Marker', 20));
  console.log(`${'─'.repeat(56)}\nqdb_form_field info-card list columns ensured.`);
}
