/**
 * DFE-GRIDSRC-001 — grid data source + display columns on qdb_form_field (additive):
 *   qdb_grid_data_source   String(20)   'entity' | 'json'
 *   qdb_grid_json_data      Memo(100000) static JSON array when data source is json
 *   qdb_grid_display_mode   String(20)   'columns' | 'infocard'
 *   qdb_grid_selectable     Boolean      read-only display when false
 *   qdb_grid_card_icon      String(100)  Fluent icon name for info-card display
 *
 * Run: node scripts/provision-grid-source-schema.mjs   (requires DV_CLIENT_SECRET)
...
const memoAttr = (schema, display, maxLength) => ({ '@odata.type': 'Microsoft.Dynamics.CRM.MemoAttributeMetadata', SchemaName: schema, LogicalName: schema, MaxLength: maxLength, RequiredLevel: { Value: 'None' }, DisplayName: label(display) });
const boolAttr = (schema, display) => ({ '@odata.type': 'Microsoft.Dynamics.CRM.BooleanAttributeMetadata', SchemaName: schema, LogicalName: schema, DefaultValue: true, RequiredLevel: { Value: 'None' }, DisplayName: label(display), OptionSet: { '@odata.type': 'Microsoft.Dynamics.CRM.BooleanOptionSetMetadata', TrueOption: { Value: 1, Label: label('Yes') }, FalseOption: { Value: 0, Label: label('No') } } });

async function run() {
  console.log(`DFE-GRIDSRC-001 schema provisioning\nOrg: ${DATAVERSE_URL}  Solution: ${SOLUTION_NAME}\n${'─'.repeat(56)}`);
