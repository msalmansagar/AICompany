/**
 * Grid paging style on qdb_form_field (additive):
 *   qdb_grid_paging_style   String(20)   'prevnext' (default) | 'numbered'
 *
 * Controls how the selection-grid pager renders: Previous/Next buttons, or
 * numbered page buttons. When unset, the runtime falls back to 'prevnext'.
 *
 * Run: node --env-file=scripts/.env scripts/provision-grid-paging-style-schema.mjs
...
async function run() {
  console.log(`Grid paging-style schema provisioning\nOrg: ${DATAVERSE_URL}  Solution: ${SOLUTION_NAME}\n${'─'.repeat(56)}`);
  const token = await acquireToken();
  await addAttribute(token, 'qdb_grid_paging_style', stringAttr('qdb_grid_paging_style', 'Grid Paging Style', 20));
  console.log(`${'─'.repeat(56)}\nqdb_form_field.qdb_grid_paging_style ensured.`);
}
