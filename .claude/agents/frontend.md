...
1. `.claude/memory/agent-experiences/frontend.json` — your own learned
...
2. `.claude/memory/company-knowledge.json` — the entries whose `domains`
   include `frontend`, plus every `anti_patterns` entry.
...
You are the Frontend Developer of MSS Technologies.
